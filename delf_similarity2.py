...
# Pace to log.
_STATUS_CHECK_LOAD_ITERATIONS = 1000
...
def _ReadAggregatedDescriptors(input_dir, image_list, config):
    ...
    for i in range(num_images):
        try :
            ...
            if config.aggregation_type == _VLAD:
                ...
                filenames.append(descriptors_filename)
            else:
    ...
    return aggregated_descriptors, visual_words, filenames
...
last_point = int(sys.argv[2])

print('Parsing dataset...from ' + str(start_point) + ',' + str(last_point))

query_list = os.listdir("d:/image/all/image_test")
index_list = os.listdir("d:/image/all/image_index")
...
query_aggregated_descriptors, query_visual_words, query_filenames = _ReadAggregatedDescriptors('output/agg/query', query_list, query_config)
index_aggregated_descriptors, index_visual_words, index_filenames = _ReadAggregatedDescriptors('output/agg/index', index_list, index_config)
...
ranks_before_gv = np.zeros([len(query_aggregated_descriptors), RANKING],dtype='int32')



for i in tqdm(range(start_point, last_point)):
    # print('Performing retrieval with query %d (%s)...' % (i, query_filenames[i]))
    start = time.clock()

    # Compute similarity between aggregated descriptors.
    similarities = np.zeros([len(index_aggregated_descriptors)])
    for j in range(len(index_aggregated_descriptors)):
    ...
    ranks_before_gv[i] = np.argsort(-similarities)[:100]
    ...
    def sibal(iter):
        ...
        test = pd.DataFrame(query_filenames)
        index = pd.DataFrame(index_filenames)

        ranks_100 = ranks.iloc[:,:100]
        new_ranks = ranks_100.apply(lambda x: x.apply(lambda y: index_filenames[int(y)].replace('.jpg.asmk_star','')), axis = 1)
        test_name = test[0].apply(lambda x: x.replace('.jpg.asmk_star',''))
        result = pd.concat([test_name, new_ranks.apply(' '.join, axis = 1) ], axis = 1)
        result.columns = ['id','images']

        result.to_csv('sample_' +str(iter) + '.csv', index = False)
        ...
        result_full.to_csv('submission_2015_06_02_iter_'+str(iter)+'.csv', index=False)
...
# result_full.to_csv('submission_2015_06_02.csv', index=False)